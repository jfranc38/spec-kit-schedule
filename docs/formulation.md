...
| $\sigma_i \in \mathbb{R}_{\geq 0}$ | Token standard deviation for task $i$ (stochastic durations) |
...
$$\text{lex-}\min\;\bigl(C_{\max},\; \mathit{TotalCost}\bigr)$$

where

$$\mathit{TotalCost} = \sum_{i \in T} \sum_{a \in \mathcal{E}_i} \left\lfloor \frac{\rho_a \cdot c_i}{1000} \right\rfloor \cdot x_{ia}$$

Same two-phase structure: Phase 1 minimises makespan; Phase 2 pins $C_{\max} \leq C_{\max}^*$ and minimises $\mathit{TotalCost}$. The solver output includes a `total_cost` field. Requires `price_per_1k_tokens` on each agent config entry.

---

## Stochastic Durations and Calibration

When tasks carry `token_std_dev` ($\sigma_i > 0$), `solver.calibrate` runs a
Monte Carlo simulation (default 1 000 samples) drawing processing times from
$p_{ia} \sim \mathcal{N}(\mu_{ia},\, (\sigma_i / \text{speed}_a)^2)$, clamped
to $[1, \infty)$. The module reports p50 and p90 makespan quantiles and
optionally writes calibrated durations back to the solver input for a
risk-adjusted schedule.

## Replanning

`solver.replan` implements online re-optimisation after partial execution.
Given a set of *frozen* assignments (completed or in-flight tasks with fixed
`(agent, start, duration)`):

1. Build the residual task set $T' \subseteq T$ (tasks not yet completed).
2. Add **frozen-start constraints**: for in-flight tasks, fix $S_i$ and $d_i$;
   if any frozen assignment is incompatible with the current portfolio, raise
   `ScheduleInputError` rather than silently dropping it.
3. The scheduling horizon expands to cover $\max_{i \in \text{frozen}}(S_i + d_i)$
   before the residual solve.
4. Re-solve CP-SAT on $(T', E \cap T' \times T')$ with the frozen tasks acting
   as resource-use constraints for their respective agents during their occupied
   intervals.
...
The benchmarks compare the CP-SAT solver against a pure-Python greedy baseline
(MAQA-style first-available-agent). For each problem, the greedy scheduler
processes tasks in topological order and assigns each task to the eligible
agent with the earliest available time, respecting κ and context-budget
constraints. The **Gap%** column shows how much the CP-SAT makespan improves
over the greedy makespan: `(greedy − cpsat) / greedy × 100`. A positive gap
means CP-SAT found a shorter schedule; zero means both methods agree.

Run `make bench` to regenerate the table below with the current solver:

<!-- benchmark table generated by benchmarks/run.py --all -->
<!-- replace this comment with output of: make bench-report -->

| Problem | n | m | Greedy makespan | CP-SAT makespan | Gap% | Greedy load_range | CP-SAT load_range | Solve time (s) |
|---------|---|---|-----------------|-----------------|------|-------------------|-------------------|----------------|
